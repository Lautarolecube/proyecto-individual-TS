...
describe("ManagerTarea", () => {
  it("validar tarea por Id devuelve true si el id existe", () => {
    ...
    manager.agregarTarea(new Tarea(1, "Desc", false, "Título", 0), 0);
  ...
  it("buscar Tareas Por Texto filtra por título o descripción", () => {
    ...
    manager.agregarTarea(
      new Tarea(1, "Aprender TypeScript", false, "Curso TS", 0),
      0
    );
    manager.agregarTarea(new Tarea(2, "Otra cosa", false, "Java", 0), 0);
  ...
  it("asignar Responsable a la tarea", () => {
    const manager = new ManagerTarea();
    manager.agregarTarea(new Tarea(1, "Aprender TypeScript", false, "Curso TS", 0), 0);
    const usuario = new User("Lautaro2", "secreto123");
